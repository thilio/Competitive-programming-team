...
struct point {
	...
	point(coord _x, coord _y, coord _z): x(_x),y(_y),z(_z) {}
 	// USE INLINE WITH EVERY OPERATOR - Saving line space
	point operator+(point p){ return {x+p.x, y+p.y, z+p.z};}
	point operator-(point p){ return {x-p.x, y-p.y, z-p.z};}
	point operator*(coord o){ return {x*o, y*o, z*o};}
	point operator/(coord o){ return {x/o, y/o, z/o};}
 	coord operator*(point p){ return x*p.x + y*p.y + z*p.z;}
	point operator^(point p){ return 
			{y*p.z - z*p.y, z*p.x - x*p.z , x*p.y - p.x*y };}
 	coord norm(){ return sqrt(x * x + y * y + z * z);}
 	coord norm2(){ return x * x + y * y + z * z; }
	bool operator==(point p){ 
return sign(x-p.x)==0 && sign(y-p.y)==0 && sign(z-p.z)==0;}
	void operator=(point p){x = p.x; y = p.y; z = p.z;}
};
//polar (degree) to cartesian in a sphere of radius r
point deg_to_cart(coord r, coord tht, coord phi){
	tht *= 2.0 * pi / 360.0; phi *= 2.0 * pi / 360.0;
	return point(cos(tht) * cos(phi), 
								cos(tht) * sin(phi), sin(tht)) * r;
}
...
point get_arc_int(coord r,point a,point b,point c,point d){
	point nrm1 = a ^ b;
	point nrm2 = c ^ d;
	point p = nrm1 ^ nrm2; // intersection of planes 
	if (sign(p.norm()) == 0)return {0, 0, 0};
	p = p * (r / p.norm());
	point pp = p * (-1.0);
