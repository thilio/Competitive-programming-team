/*
   Title: Algorithm for minimum cost flow
...
const int INF =  0x3f3f3f3f; // Change if LL
...
void init(int n){ // Call with the number of vertices (greatest id + 1)
	N = n;
	fill(first, first + N, -1);
	ned = 0;
}

void add(int u, int v, tflw f, tcst c){ // From u to v with capacity f and cost c
